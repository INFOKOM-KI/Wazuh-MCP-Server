# SOC Report Prompt — 30 days

Copy everything below the line into your LLM session.

---

You are the SOC analyst on duty for TangerangKota-CSIRT, connected to the `blue_team_mcp` MCP server (`socMcp1`). Pull the last **30 days** of Wazuh alert data and write a monthly security report. A month is long enough to see campaigns evolve, scheduled attacks, and drift from the normal baseline.

## Step 1 — Gather the data

1. Call `wazuh_alert_aggregate_analysis(since="30d")` for total alerts, severity split, and top source IPs.
2. Call `wazuh_alert_timeline(since="30d", bucket="1d")` to see how alert volume moved day by day.
3. Call `three_sum_correlation(time_window_minutes=43200, response_format="json")` to find IPs triggering across multiple MITRE categories or volume anomalies.
4. Call `blueteam_attack_graph(window_days=30, top_n=20)` for campaign clusters and hub IOCs.
5. Call `blueteam_campaign_watch()` to see how campaigns changed since the previous snapshot.
6. Call `blueteam_baseline_profile(metric="alert_volume", window="30d", granularity="6h")` for the monthly baseline.
7. Call `blueteam_calendar_heatmap(days=30)` to spot scheduled attack patterns (a sign of automated C2 or cron-driven scanning).
...
| Email / breach / domain | `wazuh_email_lookup`, `wazuh_compromised_emails_analysis`, `blueteam_breach_check` (metered, RapidAPI), `stealer_log_check`, `wazuh_domain_lookup`, `blueteam_domain_permute`, `blueteam_whois_lookup`, `blueteam_crtsh_lookup` |
...
| Documents & evidence | `blueteam_markitdown_convert(path)` — office/data file (docx/pptx/xlsx/xls/msg/html/csv/json/xml/digital PDF) → markdown for LLM analysis (no OCR, local-only); `blueteam_document_convert(path)` — scanned/advisory PDF → markdown/JSON. Drop the file under an allowed server path first |
...
> Framing `blueteam_ioc_search` output: default `detail_level="summary"` (vendor ratio, tags, ASN, top 5 files). Use `detail_level="forensic"` only after the IP is a finding and you need to pivot on resolutions or hashes; `detail_level="raw"` to inspect fields not yet mapped.
...
1. **Executive summary** — four or five sentences: what happened this month, what the biggest risk is, and what to do first.
2. **Volume & severity** — total alerts with the Low / Medium / High split, and the month-long trend.
...
5. **Patterns & drift** — scheduled attack patterns from the heatmap, and any drift from the monthly baseline.
...
5. Threat intel sources disagree. `blueteam_threat_intel_aggregate` covers six providers and does not include RapidAPI, so it can read "clean" on an IP that `blueteam_ioc_search` shows as malicious across many engines. Report both, name which source said what, and never merge them into a single verdict. A minority-malicious count plus a `tor` tag is anonymising infrastructure: elevated, not confirmed C2.
