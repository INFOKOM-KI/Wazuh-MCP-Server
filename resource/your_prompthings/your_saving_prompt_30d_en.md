...
> RapidAPI budget: 0 of 100 requests armed. Every RapidAPI product shares one account-wide pool, reserved for live incident triage, so this report must not call `blueteam_ioc_search`, `blueteam_ioc_search_bulk`, `blueteam_ip_intel_bulk`, `blueteam_breach_check`: the server refuses them with "Budget closed". Use the quota-free providers instead: `blueteam_threat_intel_aggregate` (CrowdSec, ThreatFox, AlienVault OTX, GreyNoise, AbuseIPDB, VirusTotal), `crowdsec_ip_reputation`, `threatfox_ioc_search`, `argus_ip_lookup`. A refusal is not an outage: report it once and carry on.
...
1. Fit the shape of the window: `blueteam_alert_cluster(mode="fit", time_window_minutes=43200, response_format="json")`. Report `entity_count`, `noise_ratio` and each cluster's `size`, `medoid` and `top_tactics`. `status="insufficient_data"` means the window is too small — say that, and never describe it as "no clusters found".
2. Label the alert you are actually writing about: `blueteam_incident_label(mode="alert", alert=<the alert object>, response_format="json")`. Report `label`, its `category`, `confidence` and the `floor`. `status="uncertain"` is a result — report it as uncertain and read `alternatives`; do not pick the most plausible one. If `scored=false`, the backend exposes no probabilities: report the model's choice as unscored and never invent a confidence for it. `status="unavailable"` carries a `reason`; report the reason and stop. When you run `blueteam_investigation_workflow`, this label is produced automatically for the subject alert and returned as `incident_label`; report that value instead of labeling the same text twice.
...
4. `three_sum_correlation(time_window_minutes=43200)` for the scores the cluster vector is built from, then `blueteam_rag_query` for "have we closed something like this before".
