...
1. Fit the shape of the window: `blueteam_alert_cluster(mode="fit", time_window_minutes=4320, response_format="json")`. Report `entity_count`, `noise_ratio` and each cluster's `size`, `medoid` and `top_tactics`. `status="insufficient_data"` means the window is too small — say that, and never describe it as "no clusters found".
...
4. `three_sum_correlation(time_window_minutes=4320)` for the scores the cluster vector is built from, then `blueteam_rag_query` for "have we closed something like this before".
