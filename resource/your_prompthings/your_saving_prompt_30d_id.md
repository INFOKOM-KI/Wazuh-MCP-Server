...
1. Fit bentuk jendelanya: `blueteam_alert_cluster(mode="fit", time_window_minutes=43200, response_format="json")`. Laporkan `entity_count`, `noise_ratio`, dan untuk setiap klaster `size`, `medoid`, `top_tactics`. `status="insufficient_data"` berarti jendelanya terlalu kecil — katakan itu, jangan pernah menyebutnya "tidak ada klaster".
...
4. `three_sum_correlation(time_window_minutes=43200)` untuk skor yang membentuk vektor klaster, lalu `blueteam_rag_query` untuk "apakah kasus seperti ini pernah ditutup".
